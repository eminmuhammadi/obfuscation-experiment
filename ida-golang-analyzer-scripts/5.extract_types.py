...
def parse_type(addr):
    ...
    if size > 0xFF:  # Quick & dirty sanity check.
        # This is almost certain to break eventually
        size = get_wide_byte(nameOff + 1)
        type_str = get_strlit_contents(nameOff + 2, size)
    else:
        type_str = get_strlit_contents(nameOff + 3, size)
    if not type_str:
        print(
            f"Could not obtain type name for {hex(addr)} at address {hex(nameOff)}")
    ...
    set_name(addr, "type_" + type_str.decode(errors="replace")
             [:20], SN_NOCHECK | 0x800)
    return True
